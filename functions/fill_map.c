...
/*   Updated: 2024/02/03 19:21:21 by jlu              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (mlx_image_to_window(game->mlx_ptr, \
	game->img->monster, x * PIXELS, y * PIXELS) < 0)
...
{
	if (mlx_image_to_window(game->mlx_ptr, \
	game->img->player_right, x * PIXELS, y * PIXELS) < 0)
		error_msg_params("Failed to put image to Player");
	if (mlx_image_to_window(game->mlx_ptr, \
	game->img->player_up, x * PIXELS, y * PIXELS) < 0)
		error_msg_params("Failed to put image to Player");
	if (mlx_image_to_window(game->mlx_ptr, \
	game->img->player_left, x * PIXELS, y * PIXELS) < 0)
		error_msg_params("Failed to put image to Player");
	if (mlx_image_to_window(game->mlx_ptr, \
	game->img->player_down, x * PIXELS, y * PIXELS) < 0)
		error_msg_params("Failed to put image to Player");
	if (mlx_image_to_window(game->mlx_ptr, \
	game->img->player_die, x * PIXELS, y * PIXELS) < 0)
		error_msg_params("Failed to put image to Player");
	clear_player(game);
...
void	select_image(t_game *game, size_t y, size_t x)
{
	if (game->map[y][x] == '1')
		if (mlx_image_to_window(game->mlx_ptr, \
		game->img->wall, x * 32, y * 32) < 0)
			error_msg_params("Failed to put image to Wall");
	if (game->map[y][x] == 'C')
		if (mlx_image_to_window(game->mlx_ptr, \
		game->img->chest_closed, x * 32, y * 32) < 0)
			error_msg_params("Failed to put image to Chest");
	if (game->map[y][x] == 'P')
		if (mlx_image_to_window(game->mlx_ptr, \
		game->img->player, x * 32, y * 32) < 0)
			error_msg_params("Failed to put image to Player");
	if (game->map[y][x] == 'E')
		if (mlx_image_to_window(game->mlx_ptr, \
		game->img->exit_closed, x * 32, y * 32) < 0)
			error_msg_params("Failed to put image to Exit");
	if (game->map[y][x] == 'X')
		if (mlx_image_to_window(game->mlx_ptr, \
		game->img->monster, x * 32, y * 32) < 0)
			error_msg_params("Failed to put image to Monster");
}
...
void	fill_map(t_game *data)
{
	...
	while (y < data->height)
	{
		...
		while (x < data->width)
		{
			if (mlx_image_to_window(data->mlx_ptr, \
			data->img->floor, x * 32, y * 32) < 0)
