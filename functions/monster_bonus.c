...
/*   Updated: 2024/02/04 18:22:51 by jlu              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	check_touched(t_game *game)
{
	...
	{
		if (game->player_x * 32 == (size_t)game->img->monster->instances[i].x \
		&& game->player_y * 32 == (size_t)game->img->monster->instances[i].y)
		{
			clear_player(game);
			game->img->player_die->enabled = true;
			game->img->you_lose = mlx_put_string(game->mlx_ptr, "You Lose :(", \
			((game->width - 2) * 32) / 2, ((game->height - 1) * 32) / 2);
			game->condition = 2;
...
void	monster_move(t_game *game, int count)
{
	...
	var.x = &game->img->monster->instances[count].x;
	var.y = &game->img->monster->instances[count].y;
	...
	if (var.idx_y != 0)
		var.idx_y /= PIXELS;
	if (game->map[var.idx_y][var.idx_x] != '1' \
	&& game->map[var.idx_y][var.idx_x] != 'E')
	{
		*var.x += var.ran_x;
		*var.y += var.ran_y;
	}
...
void	end_game(mlx_key_data_t key, void *data)
{
	...
	else
	{
		...
		{
			mlx_close_window(game->mlx_ptr);
			ft_putstr_fd("WoW you are amazing!\n", 1);
		}
	}
}
